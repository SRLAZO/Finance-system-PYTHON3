...
#------------VENTANA PARA CREAR GASTOS------------------------------------------------------------------

class CreateGastoWindow(Toplevel, CenterWidgetMixin):
    def __init__(self, parent):
        super().__init__(parent)

        self.title("Crear gasto")
        self.build()
        self.center()
    ...
    def build(self):
        ...
        crear = Button(frame, text="Crear", command=self.create_gasto, width=8)
    ...
    def create_gasto(self):
        ...
        db.Gastos.crear(self.dni.get(), self.nombre.get(), self.monto.get(), self.frecuencia.get())
    ...
    def validate(self, event, index):
        ...
        valido = helpers.dni_valido(valor, db.Gastos.lista) if index == 0 \
...
#-------------VENTADA PARA MODIFICAR GASTOS-------------------------------------------------------

class EditGastoWindow(Toplevel, CenterWidgetMixin):
    def __init__(self, parent):
        super().__init__(parent)

        self.title("Actualizar gasto")
        self.build()
        self.center()
        self.transient(parent)
        self.grab_set()
    ...
    def build(self):
        ...
        gasto = self.master.treeview.focus()
        campos = self.master.treeview.item(gasto, 'values')
        dni.insert(0, campos[0])
        dni.config(state=DISABLED)
        nombre.insert(0, campos[1])
        monto.insert(0, campos[2])
        frecuencia.insert(0, campos[3])

        frame = Frame(self)
        frame.pack(pady=10)

        actualizar = Button(frame, text="Actualizar", command=self.edit_gasto, width=8)
    ...
    def edit_gasto(self):
        gasto = self.master.treeview.focus()
        self.master.treeview.item(gasto, values=(
            self.dni.get(), self.nombre.get(), self.monto.get(), self.frecuencia.get()))
        db.Gastos.modificar(self.dni.get(), self.nombre.get(), self.monto.get(), self.frecuencia.get())
        self.close()
...
#-------VENTANA PRINCIPAL GASTOS-----------------------------------------------------

class MainWindow(Tk, CenterWidgetMixin):
    def __init__(self):
        super().__init__()
        self.title("FLITCHI - GASTOS")
        self.build()
        self.center()
    ...
    def build(self):
        ...
        for gasto in db.Gastos.lista:
            treeview.insert(
                parent='', index='end', iid=gasto.dni,
                 values=(gasto.dni, gasto.nombre, gasto.monto, gasto.frecuencia))
    ...
    def delete(self):
        gasto = self.treeview.focus()
        if gasto:
            campos = self.treeview.item(gasto, "values")
            confirmar = askokcancel(
                title="Confirmar borrado",
                message=f"¿Borrar {campos[1]} {campos[2]}?",
                icon = WARNING)
            if confirmar:
                self.treeview.delete(gasto)
                db.Gastos.borrar(campos[0])

    def create(self):
        CreateGastoWindow(self)


    def edit(self):
        if self.treeview.focus():
            EditGastoWindow(self)
